...
async function validate (id) {
  ...
  try {
    ...
    if (error.stderr?.includes('code ENOVERSIONS')) {
      ...
      error.stderr?.includes('no such package available') ||
      error.stderr?.includes('code ERESOLVE')
    ) {
      ...
      return
    } else if (error.stderr?.includes('[ERR_REQUIRE_ESM]')) {
      log(id, 'INVALID: Imports ESM with require()')
      return
      ...
      error.stderr?.includes('SyntaxError: ')
    ) {
      log(id, 'Package unusable after install')
      return
    } else if (error.stderr?.includes('Unknown file extension')) {
      log(id, 'Package entry not JavaScript')
      return
